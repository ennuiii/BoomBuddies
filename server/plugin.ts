...
class GameTemplatePlugin implements GamePlugin {
  ...
  private videoPeers = new Map<string, Set<string>>(); // roomCode -> Set of socketIds with video enabled
  ...
  onPlayerDisconnected(room: Room, player: Player): void {
    ...
    // Remove from video peers and notify others
    this.removeFromVideoPeers(room.code, player.socketId);
  ...
  onPlayerLeave(room: Room, player: Player): void {
    ...
    // Remove from video peers (in case disconnect handler didn't catch it)
    this.removeFromVideoPeers(room.code, player.socketId);
  ...
  onRoomDestroy?(room: Room): void {
    ...
    // Clean up video peers for this room
    this.videoPeers.delete(room.code);
  }
  ...
  socketHandlers: Record<string, SocketEventHandler> = {
    ...
    /**
     * Player ready for video chat - notify others and send existing peers
     */
    'webrtc:ready': async (socket: Socket, data: { roomCode: string }, room: Room, helpers: GameHelpers) => {
      try {
        const player = Array.from(room.players.values()).find(p => p.socketId === socket.id);
        if (!player) return;

        console.log(`[${this.name}] WebRTC: Player ${player.name} ready for video in room ${room.code}`);

        // Initialize video peers set for this room if needed
        if (!this.videoPeers.has(room.code)) {
          this.videoPeers.set(room.code, new Set());
        }

        const roomVideoPeers = this.videoPeers.get(room.code)!;

        // Tell existing video peers about this new peer
        roomVideoPeers.forEach(existingPeerId => {
          if (existingPeerId !== socket.id) {
            helpers.sendToPlayer(existingPeerId, 'webrtc:peer-joined', {
              peerId: socket.id,
        ...
        // Tell the new peer about all existing video peers
        roomVideoPeers.forEach(existingPeerId => {
          if (existingPeerId !== socket.id) {
            const existingPlayer = Array.from(room.players.values()).find(p => p.socketId === existingPeerId);
            if (existingPlayer) {
              socket.emit('webrtc:peer-joined', {
        ...
        // Add this player to video peers
        roomVideoPeers.add(socket.id);

      } catch (error) {
        console.error(`[${this.name}] WebRTC ready error:`, error);
      }
    },

    /**
     * Player left video chat
     */
    'webrtc:leave': async (socket: Socket, data: { roomCode: string }, room: Room, helpers: GameHelpers) => {
      try {
        const player = Array.from(room.players.values()).find(p => p.socketId === socket.id);
        console.log(`[${this.name}] WebRTC: Player ${player?.name || socket.id} left video in room ${room.code}`);

        // Remove from video peers
        const roomVideoPeers = this.videoPeers.get(room.code);
        if (roomVideoPeers) {
          roomVideoPeers.delete(socket.id);

          // Notify other peers
          roomVideoPeers.forEach(peerId => {
            helpers.sendToPlayer(peerId, 'webrtc:peer-left', {
              peerId: socket.id
            });
          });
        }
        ...
        console.error(`[${this.name}] WebRTC leave error:`, error);
      }
    },
     ...
     */
    'webrtc:offer': async (socket: Socket, data: { targetId: string; offer: any }, room: Room, helpers: GameHelpers) => {
      try {
        console.log(`[${this.name}] WebRTC: Relaying offer from ${socket.id} to ${data.targetId}`);
        helpers.sendToPlayer(data.targetId, 'webrtc:offer', {
          senderId: socket.id,
          offer: data.offer
        });
      } catch (error) {
        console.error(`[${this.name}] WebRTC offer relay error:`, error);
      }
    },
     ...
     */
    'webrtc:answer': async (socket: Socket, data: { targetId: string; answer: any }, room: Room, helpers: GameHelpers) => {
      try {
        console.log(`[${this.name}] WebRTC: Relaying answer from ${socket.id} to ${data.targetId}`);
        helpers.sendToPlayer(data.targetId, 'webrtc:answer', {
          senderId: socket.id,
          answer: data.answer
        });
      } catch (error) {
        console.error(`[${this.name}] WebRTC answer relay error:`, error);
      }
    },
     ...
     */
    'webrtc:ice-candidate': async (socket: Socket, data: { targetId: string; candidate: any }, room: Room, helpers: GameHelpers) => {
      try {
        helpers.sendToPlayer(data.targetId, 'webrtc:ice-candidate', {
          senderId: socket.id,
          candidate: data.candidate
        });
      } catch (error) {
        console.error(`[${this.name}] WebRTC ICE candidate relay error:`, error);
      }
    }
  };
  ...
  /**
   * Remove a player from video peers and notify others
   */
  private removeFromVideoPeers(roomCode: string, socketId: string): void {
    const roomVideoPeers = this.videoPeers.get(roomCode);
    if (!roomVideoPeers || !roomVideoPeers.has(socketId)) return;

    roomVideoPeers.delete(socketId);

    // Notify remaining peers that this player left video
    if (this.io) {
      const namespace = this.io.of(this.namespace);
      roomVideoPeers.forEach(peerId => {
        namespace.to(peerId).emit('webrtc:peer-left', {
          peerId: socketId
        });
      });
    }

    console.log(`[${this.name}] Removed ${socketId} from video peers in room ${roomCode}`);
  }
}
